...
public class MatchSummaryPageElements extends KeyElements {
	...
	protected MatchSummaryPageElements(WebDriver webDriver, String url) {
		...
		try {
			webDriver.findElements(By.xpath(elementIdentifier
					.getString("matchaction")));
	...
	protected WebElement getAMatchActionExclamationICon(int i) {
		...
		return webDriver.findElement(By.xpath("(//div[@class='match-action'])["
				+ i + "]/div[@class='single-action-flags']/input[1]"));
	...
	protected WebElement getAMatchActionFavouriteICon(int i) {
		...
		return webDriver.findElement(By.xpath("(//div[@class='match-action'])["
				+ i + "]/div[@class='single-action-flags']/input[2]"));
	...
	protected WebElement getAMatchActionGameChangingICon(int i) {
		...
		return webDriver.findElement(By.xpath("(//div[@class='match-action'])["
				+ i + "]/div[@class='single-action-flags']/input[3]"));
	...
	protected WebElement getActionForYellowCard(int actionNo) {
		...
		try {
			return webDriver
					.findElement(By
							.xpath("(//div[@class='match-action']/div[@class='action-icon booking'])[position()="
									+ getActionNo() + "]"));
		} catch (NoSuchElementException e) {
			e.printStackTrace();
		}
